...
async function ask(handle, prompt) {
  ...
  hbStart = setTimeout(() => {
    try { sendiMessage('still working...', handle); } catch {  }
...
function newMessages(sinceRowId) {
  ...
  for (const ext of ['', '-wal', '-shm']) {
    if (existsSync(CHAT_DB + ext)) { try { copyFileSync(CHAT_DB + ext, tmp + ext); } catch {  } }
  }
...
async function tick() {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (!isOwner(row.handle)) { console.log(`(skip msg from ${row.handle})`); continue; }
      if (!row.text) continue;
      if (wasSentByUs(row.text)) continue;
      ...
      try {
        ...
        try { sendiMessage(m, row.handle); } catch {  }
        console.error(e);
      }
    }
  } catch (e) {
