#!/usr/bin/env node
// Report the screen size (virtual desktop across all monitors) so Helm can compute coordinates
// for gui.click / gui.move. Read-only.
import { screenBounds } from './win-input.mjs';

if (process.platform === 'darwin') {
  const { spawnSync } = await import('node:child_process');
  // points, not pixels — matches gui.click's coordinate space on macOS
