...
const buildPrompt = t => [
  ...
  `- CRITICAL ISOLATION: your current working directory IS your sandbox copy of the repo. Create/edit files with RELATIVE paths only. NEVER cd elsewhere, and NEVER write to absolute paths like /Users/owner/secondme or ~/secondme — that corrupts the real repo and fails you. (Absolute paths appearing as config DATA, e.g. an MCP filesystem root, are fine to write into a config file.)`,
  `- NEVER touch ~/helm, the Helm Supabase project, or com.helm.agent.`,
