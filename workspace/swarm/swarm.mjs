...
const buildPrompt = t => [
  ...
  `- CRITICAL ISOLATION: your current working directory IS your sandbox copy of the repo. Create/edit files with RELATIVE paths only. NEVER cd elsewhere, and NEVER write to the real install directory via an absolute path — that corrupts the real repo and fails you. (Absolute paths appearing as config DATA, e.g. an MCP filesystem root, are fine to write into a config file.)`,
