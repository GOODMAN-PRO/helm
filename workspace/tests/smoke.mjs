...

{
  ...
  try {
    ...
    try { result = JSON.parse(result).result ?? result; } catch {  }
...

{
  ...
  try {
    ...
    const atUtc2 = new Date(Date.UTC(2026, 4, 30, 2, 0, 0));
    if (!cronMatches('0 2 * * *', atUtc2)) throw new Error('must match UTC 02:00');
    const atUtc9 = new Date(Date.UTC(2026, 4, 30, 9, 0, 0));
...

{
  ...
  try {
    ...
    const cases = [
      ...
      ['ok',                         'haiku'],
      ['thanks',                     'haiku'],
      ['build a discord bot',        'opus'],
      ['implement oauth login',      'opus'],
      ['debug this crash',           'opus'],
      ['fix the bug in auth.js',     'opus'],
      ['create a REST api endpoint', 'opus'],
      ['x'.repeat(1001),             'opus'],
      ['explain the difference between TCP and UDP protocols', 'sonnet'],
    ];
...

{
  ...
  try {
    ...
    recordStuck(PROBE + '!', 'detail b', 'test');
...

{
  ...
  try {
    ...
    const pos = new Map(done.map((id, i) => [id, i]));
